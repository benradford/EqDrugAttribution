...
```{r, message=F, error=F, warning=F, cache=F}
library(timeDate)
library(XML)
library(stringr)
...
countries <- tolower(c("US","UK","RUSSIA","IRAN","CHINA","BRAZIL","CANADA","ISRAEL"))
...
```{r, echo=T, error=F, message=F, warning=F, results='hold', fig.width=6, fig.height=6, fig.align='center'}
...
1. Link dates for EquationDrug malware are randomly chosen from the work week.
2. Equation Group observes, in general, official holidays of the state in which they operate.
3. The timestamps given by Kaspersky are accurate.
    + Note that there are many reasons that this assumption could be wrong. First, Equation Group could obscure the timestamps. More importantly, the computer's time zone might not match the programmer's time zone. Unfortunately, without the full timestamp data for each piece of malware, I cannot think of a way to compensate adequately for this.
